    ...
    <div className="px-4 lg:px-8 py-5">
      ...
      {error && (
        <div className="card p-8 text-center">
          ...
          <p className="text-gray-700 dark:text-gray-300 font-medium">Dados não disponíveis</p>
                ...
                <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
                  <tr>
                    {Object.keys(data[0]).map((col) => (
                      <th
                        key={col}
                        className="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide whitespace-nowrap"
                      >
                        {col}
                      </th>
                    ))}
                  ...
                  {data.map((row, i) => (
                    <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      {Object.values(row).map((val, j) => (
                        <td key={j} className="px-4 py-2.5 text-gray-800 dark:text-gray-200 whitespace-nowrap">
                          <CellValue value={val} />
                        </td>
                      ))}
                    </tr>
